...
function handleSnapClick(event)
{
	...
	C64Module.loadSnapshot( document.getElementById("selectGame").value, false );
	return false;
}
...
function handleSnapJoy()
{
	...
	if( joyTest.innerHTML != joyTest.getAttribute('data-back') ) {
		joyTestSave = C64Module.saveSnapshotData();
		C64Module.loadSnapshot( 'Joystick_Test.s64', false );
		joyTest.innerHTML = joyTest.getAttribute('data-back');
	}
	else {
		if(joyTestSave) C64Module.loadSnapshotData( joyTestSave );
		joyTestSave = null;
...
function handleJoyConf(event)
{
	...
	C64Module.selectJoystick( joy.value&0xff, joy.value>>8 );
	joy.blur();
	return false;
}
...
function handleHighscore(event)
{
	...
	C64Module.uploadHighscore(userName, scoreId, snapName);
	return false;
}
  ...
  if( dt.items ) {
	...
	for (var i=0; i < dt.files.length; i++) {
		if( c64_drag_checkfile(dt.files[i])) {
			C64Module.loadFile(dt.files[i], startup); 
...
function c64_handleDiskInput(event) 
{
  ...
  for (var i=0; i < myfiles.length; i++) {
	  if( c64_drag_checkfile(myfiles[i])) {
			C64Module.loadFile(myfiles[i], startup); 
...
function process_touch(ev) {
	if( 0==touchEnabled ) {
		touchEnabled = 1;
		C64Module.setProperty( 'touchtype', touchType.toString() );
		if(touchType == 1) {
...
function addControlHandler(parent)
{
	const ctrl =
	{
		cmd_fs: function() { C64Module.requestC64FullScreen(); },
		cmd_tm: function() { C64Module.toggleMute(); },
		cmd_ss: function() { C64Module.snapshotStorage(1); },
		cmd_sl: function() { C64Module.snapshotStorage(0); },
		cmd_uh: function() { C64Module.uploadHighscore('', '', 'self'); }
	};
...
function addButtonHandler(parent)
{
	...
	for (var i = 0; i < children.length; i++)
	{
		...
		if(child.nodeName == "BUTTON")
		{
			...
			{
				child.addEventListener("mousedown", function(e) { C64Module.setKey(this.name.substr(4), 1); }, false);
				child.addEventListener("mouseup", function(e) { C64Module.setKey(this.name.substr(4), 0); }, false);

				child.addEventListener("touchstart", function(e) { C64Module.setKey(this.name.substr(4), 1); if(e.preventDefault) e.preventDefault(); }, false);
				child.addEventListener("touchcancel", function(e) { C64Module.setKey(this.name.substr(4), 0); if(e.preventDefault) e.preventDefault(); }, false);
				child.addEventListener("touchend", function(e) { C64Module.setKey(this.name.substr(4), 0); if(e.preventDefault) e.preventDefault(); }, false);
			}
			else if(child.name.startsWith('joy_'))
			{
				child.addEventListener("mousedown", function(e) { C64Module.setJoystick(this.name.substr(4), 1); }, false);
				child.addEventListener("mouseup", function(e) { C64Module.setJoystick(this.name.substr(4), 0); }, false);
				
				child.addEventListener("touchstart", function(e) { C64Module.setJoystick(this.name.substr(4), 1); if(e.preventDefault) e.preventDefault(); }, false);
				child.addEventListener("touchcancel", function(e) { C64Module.setJoystick(this.name.substr(4), 0); if(e.preventDefault) e.preventDefault(); }, false);
				child.addEventListener("touchend", function(e) { C64Module.setJoystick(this.name.substr(4), 0); if(e.preventDefault) e.preventDefault(); }, false);
			} 
			else if(child.name.startsWith('tape_'))
			{
				child.addEventListener("mousedown", function(e) { C64Module.setTape(this.name.substr(5)); }, false);
				
				child.addEventListener("touchstart", function(e) { C64Module.setTape(this.name.substr(5)); if(e.preventDefault) e.preventDefault(); }, false);
			} 
		}
		else
		{
...
function addConfigHandler(parent)
{
	...
	document.getElementById('config_inverty').addEventListener("click", function(e) {
		...
		C64Module.setProperty("joy_inverty", check.checked?"48":"0");
		}, false);
	...
	document.getElementById('config_fastload').addEventListener("click", function(e) {
		...
		C64Module.setProperty("disk_fast_loading", check.checked?"1":"0");
		}, false);
	...
	document.getElementById('config_wrprotect').addEventListener("click", function(e) {
		...
		C64Module.setProperty("disk_write_protection", check.checked?"2":"0");
		}, false);

	document.getElementById('config_reset').addEventListener("click", function(e) {
		C64Module.resetEmulation(1);
		}, false);
...
{
	document.getElementById('print_version').innerHTML = 'V'+C64Module.getProperty('version')+'/'+C64Module.getProperty('revision');
	document.getElementById('config_inverty').checked = C64Module.getProperty("joy_inverty") != '0';
	document.getElementById('config_fastload').checked = C64Module.getProperty("disk_fast_loading") != '0';
	document.getElementById('config_wrprotect').checked = C64Module.getProperty("disk_write_protection") != '0';
	document.getElementById('config_touch').checked = C64Module.getProperty("touchtype") == '2';
}
...
var arrow_keys_handler = function(e) {
    switch(e.keyCode){
		...
		if( C64Module.hasFocus ) e.preventDefault();
		break;
        default: break;
    }
};
