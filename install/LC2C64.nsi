...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2C64"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "LC2C64 Client"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2C64"
...
Name "LC2C64"
Caption "LC2C64 Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2C64install.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2C64"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2C64" "LC2C64"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2C64.lnk" $INSTDIR\LC2C64.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2C64 "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2C64" "DisplayName" "LC2C64 (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2C64" "UninstallString" '"$INSTDIR\uninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2C64.exe"
  ;File /a "exe\*.*"
  File /a ".\install.zip"
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2C64 "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2C64 "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2C64 "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2C64 "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2C64 "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\Start.LC2Launcher.lnk" $INSTDIR\LC2C64.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\LC2C64.lnk" $INSTDIR\LC2C64.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\_LC2Launcher.lnk" $INSTDIR\LC2C64.exe
 CreateShortCut "$INSTDIR\Update.lnk" $INSTDIR\LC2Start.NET.exe
 CreateShortCut "$DESKTOP\\LC2C64.exe.lnk" $INSTDIR\LC2Start.NET.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\AutoStart.lnk" $INSTDIR\LC2autostart.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\AutoStartConfig.lnk" $INSTDIR\LC2autostartConfig.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\AutoStartUninstall.lnk" $INSTDIR\LC2autostartUninstall.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\LC2Config.lnk" $INSTDIR\LC2Config.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\Uninstall.lnk" $INSTDIR\uninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
UninstallText "Uninstall LC2C64 ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2C64"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2C64"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2C64\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  ...
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\LC2C64.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\uninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2C64\*.*"
